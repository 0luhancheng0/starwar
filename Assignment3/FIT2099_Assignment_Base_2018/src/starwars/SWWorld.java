...
public class SWWorld extends World {
	...
	private SWLocation.SWLocationMaker factory;
	...
	void setDefaultLocString(SWLocation loc) {
	...
	public void initializeWorld(MessageRenderer iface) {
		...
		for (int i = 0; i < 10; i++) {
			...
			grenade.setSymbol("G");
			loc = myGrid.getLocationByCoordinates(randomGenerator.nextInt(10), randomGenerator.nextInt(10));
	...
	/**
	 * Return a arraylist collection of the entities that has different distance to the <code>targetLoc</code> 
	 * the distance range from 0 to <code>step</code>
	 * @param targetLoc is the central location
	 * @return a arraylist collection of the entities that has different distance to the <code>targetLoc</code>  
	 */
	public static ArrayList<ArrayList<ArrayList<SWEntityInterface>>> getNeighboursContents(SWLocation targetLoc, int step) {
		
		//get all the locations within the steps from targetLoc
		ArrayList<ArrayList<SWLocation>> neighboursLoc = targetLoc.getNeighboursLoc(step);
		
		//initialize the list that is goint to store the result
		ArrayList<ArrayList<ArrayList<SWEntityInterface>>> neighboursContents = new ArrayList<ArrayList<ArrayList<SWEntityInterface>>>();
		
		//iterate through all the steps
		for (ArrayList<SWLocation> locs : neighboursLoc) {
			
			//create the collection of entities from each step
			ArrayList<ArrayList<SWEntityInterface>> temp = new ArrayList<ArrayList<SWEntityInterface>>();
			
			//iterate through all the location for the step
			for (SWLocation loc: locs) {
				
				//get all the entities in the location
				 ArrayList<SWEntityInterface> contentsSingleLoc = (ArrayList<SWEntityInterface>) SWWorld.entityManager.contents(loc);
				 
				 //it is possible that the location does not contain any entity
				 if (contentsSingleLoc != null) {
					 temp.add(contentsSingleLoc);
				 }
			}
			
			neighboursContents.add(temp);
		}
		
		return neighboursContents;
